...
public class Warlord extends Character {
    ...
    private void destroy() {
        if (CharactersList.allCharacterCards[Role.BISHOP.ordinal()].isPlayed() && !CharactersList.allCharacterCards[Role.BISHOP.ordinal()].isDead()) {
            this.getPlayer().getMemory().getDisplay().addBishopPower();
        }
        if (DistrictsPile.allDistrictCards[Unique.KEEP.toIndex()].isBuilt() && !DistrictsPile.allDistrictCards[Unique.KEEP.toIndex()].getOwner().equals(CharactersList.allCharacterCards[Role.BISHOP.ordinal()].getPlayer())) {
        ...
        if (!DistrictsPile.allDistrictCards[Unique.GRAVEYARD.toIndex()].useEffect()) {
